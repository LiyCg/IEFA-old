...
class FacialMotion():
    def __init__(self, animation_seq : np.array = None):
        ...
        self.default_intensity_val = 0.5 
        self.key_idx = 0 
        self.intensity_vector = np.ones(53) * self.default_intensity_val # (53,) vector set to 0.5 as default
    ...
    def set_speed(self, speed_locations : list, speed_frames : list, replace=False): 
        for speed_location in speed_locations:
            assert(speed_location in ["front", "keyframes", "back"])
    ...
    def set_key_exp(self, intensities : list = None): 
        ...
        assert np.all(self.key_exp_parameter >= 0.0) and np.all(self.key_exp_parameter <= 1.0), "key_exp_parameter should be in the range [0.0,1.0]" 
    ...
    def activate_hierarchical(self, identifiers : list, intensities: list = None):
        ...
        if intensities == None:
            ...
            for _ in range(len(identifiers)):
                intensities.append(self.default_intensity_val)
    ...
    def insert_keyframe(self): # interpolate 잘 안되는중 디버깅 잘 살펴보기, 아무래도 precision 때문인 것 같음. 너무 잘게 나눠서 interpolate 값이 smooth하게 적용이 안됨
        """
        ...
        for i in range(duration):
            ...
            if index < 0 or index >= self.seq_len:
                continue
        ...
        if start_idx - self.speed["front"] <= 0: # check if enough front_speed frames are available, if not, decrease front_speed to allowalbe length
            self.speed["front"] = start_idx - 1
            front_speed_frame = output_sequence[0]
        else:
            front_speed_frame = output_sequence[start_idx - (self.speed["front"] + 1)]
        
        for i in range(1, self.speed["front"] + 1):
            if start_idx - i < 0:
                break
            t = i / (self.speed["front"] + 1)
            output_sequence[start_idx - i] = t * (front_speed_frame) + (1-t) * (output_sequence[start_idx])
            
        if end_idx + self.speed["back"]  >= self.seq_len - 1: # check if enough back_speed frames are available, if not, decrease back_speed to allowalbe length
            self.speed["back"] = self.seq_len - (end_idx - 1) - 1
            back_speed_frame = output_sequence[-1]
        else:
            back_speed_frame = output_sequence[end_idx + (self.speed["back"]  + 1)]
        
        for i in range(1, self.speed["back"]  + 1):
            if end_idx + i >= self.seq_len:
                break  
            t = i / (self.speed["back"]  + 1)
...
if __name__ == "__main__":
    ...
    audio_path = "./"
    motion_0 = FacialMotion() 
    identifiers = ["brow_outer_up_r","brow_outer_up_l","brow_inner_up_r"]
    identifiers = ["happy","angry"]
    identifiers = ["outer_brow_raiser", "cheek_raiser"]
    intensities = [0.8,0.7]
    motion_0.activate_hierarchical(identifiers=identifiers, intensities=intensities)
    motion_0.set_insert_point(0.5)
    motion_0.set_speed(["keyframes"], [50], replace=True)
    motion_0.activate_blendshape()
    print(motion_0.output_animation_seq[30])
